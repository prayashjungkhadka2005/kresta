...
import { ArrowLeft, Package, ExternalLink, Copy, Check, Zap, TrendingUp, MousePointerClick, ShoppingBag } from "lucide-react";
...
export default function MarketplaceProductDetailPage() {
    ...
    if (notFound) {
        return (
            <div className="space-y-6 max-w-7xl mx-auto w-full pb-10">
                <Link href="/creator/marketplace" className="flex items-center gap-2 text-sm text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white transition-colors font-bold">
            ...
            <div className="space-y-6 max-w-7xl mx-auto w-full pb-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-4">
                        {[1, 2, 3, 4, 5].map(i => (
                            <div key={i} className={`h-${i === 1 ? 6 : 4} bg-gray-100 dark:bg-zinc-800 rounded animate-pulse`} />
                        ))}
                    </div>
                </div>
            </div>
    ...
    const coverImages = product.media.filter(m => m.type === "IMAGE");
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto w-full pb-10">
            ...
            <Link
                ...
                className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-zinc-500 hover:text-gray-900 dark:hover:text-white transition-colors"
            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
                ...
                <div className="flex flex-col gap-3 h-full">
                    <div className="relative bg-gray-50 dark:bg-zinc-900/50 border border-gray-100 dark:border-zinc-800 rounded-xl overflow-hidden flex-1 min-h-[320px]">
                        {coverImages.length > 0 ? (
                            <img
                                src={coverImages[selectedImage]?.url || coverImages[0].url}
                                alt={product.name}
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-1">
                            {coverImages.map((img, i) => (
                                <button
                                    ...
                                    className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors ${selectedImage === i ? "border-gray-900 dark:border-white" : "border-gray-100 dark:border-zinc-800 opacity-60 hover:opacity-100"}`}
                                >
                    ...
                    {isLoadingLink ? (
                        ...
                        <div className="space-y-4">
                            <div className="bg-gray-50 dark:bg-zinc-800/50 border border-dashed border-gray-200 dark:border-zinc-700 rounded-xl p-5 text-center space-y-3">
                                ...
                                >
                                    {isGenerating ? "Generating..." : "Generate My Affiliate Link"}
                                </Button>
                            </div>
                        </div>
                    )}
